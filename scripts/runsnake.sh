...
#cat <<EOF
$qsub -N ${JOBNAME}.snakemake -o ${JOBNAME}.snakemake.qsub.log "snakemake --jn \"${JOBNAME}.skj.{rulename}.{jobid}.sh\" -j ${NUMJOBS} -c \"qsub -pe OpenMP {threads} -l mem_free=50G -l h_rt=${RUNTIME}:00:00 -j y -V -b y -cwd\" -s ${SMAKE} --configfile ${SCONF} --config ${CONFIGVAR} -w 100 -d ${WORKDIR}";
#EOF

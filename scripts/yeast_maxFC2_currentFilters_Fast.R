...
group_names[!is.na(match(sampleInfo[,"LibID"],conditionLibIDs))] <- TRUE
...
if(NROW(output)>0){
  ...
  evaluefilter = rep("nopass",NROW(output));
  evaluefilter[output[,E_value]<10]="pass";
  output[E_value<10,evaluefilter:="pass"];
  # only keep those passed output
  v1all  = v1all[chr %in% output[evaluefilter=="pass",chr],];
  etTable  = etTable[chr %in% output[evaluefilter=="pass",chr],];
  output[,rowid:=.I];
  output[,mapply(testlist(.SD)),by = rowid];
...
#    min2[i] <- median(et[is.finite(et[[2]]),2])+sd(et[is.finite(et[[2]]),2]) this criteria is too strigent so that orf19.6854 can not pass the fold change criteria
    ...
    et[,c("pvaluefilter","covfilter"):=.(
      ifelse(PValue<thr,"pass","nopass"),
      ifelse(is.finite(logCPM) & logCPM > quantile(logCPM,0.65),"pass","nopass"))];

...
p1 <- as.character(unlist(mapply(rep,names(l),l))) # the chromosome id for each site
p2 <- as.character(unlist(lapply(l,function(x) c(1:x)))) # the genomic coordindate for each site

for (i in conditions){
  print(i)
  load(paste(resultdir,"etTable_",i,".Rdata",sep=""))
